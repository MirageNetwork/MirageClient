...
func (t *winTray) showSeparatorItem(menuItemId, parentId uint32) error {
	const (
		MIIM_FTYPE   = 0x00000100
		MIIM_SUBMENU = 0x00000004
		MIIM_ID      = 0x00000002
...
func showHideSeparatorItem(item *SeparatorItem) {
	err := wt.showSeparatorItem(uint32(item.id), item.parentId())
	if err != nil {
		log.Errorf("Unable to showSeparatorItem: %v", err)
		return
	}
}
...
func addSeparator(id uint32, pid uint32) uint32 {
	err := wt.addSeparatorMenuItem(id, pid)
	if err != nil {
...
func (item *SeparatorItem) parentId() uint32 {
	if item.parent == nil {
		return 0
	}
...
func hideSeparatorItem(item *SeparatorItem) {
	err := wt.hideMenuItem(uint32(item.id), item.parentId())
	if err != nil {
		log.Errorf("Unable to hideMenuItem: %v", err)
		return
...
func showSeparatorItem(item *SeparatorItem) {
	showHideSeparatorItem(item)
}
