...
func (ft *firewallTweaker) doSet(local []string, killswitch bool, clear bool, procRule bool, allowedRoutes []netip.Prefix) error {
	if clear {
		ft.logf("clearing Mirage-In firewall rules...")
		...
		d, _ := ft.runFirewall("delete", "rule", "name=Mirage-In", "dir=in")
		ft.logf("cleared Mirage-In firewall rules in %v", d)
	}
	if procRule {
		ft.logf("deleting any prior Mirage-Process rule...")
		d, err := ft.runFirewall("delete", "rule", "name=Mirage-Process", "dir=in") // best effort
		if err == nil {
			ft.logf("removed old Mirage-Process rule in %v", d)
		}
		var exe string
		exe, err = os.Executable()
		if err != nil {
			ft.logf("failed to find Executable for Mirage-Process rule: %v", err)
		} else {
			ft.logf("adding Mirage-Process rule to allow UDP for %q ...", exe)
			d, err = ft.runFirewall("add", "rule", "name=Mirage-Process",
				"dir=in",
				"action=allow",
				"edge=yes",
				"program="+exe,
			...
			if err != nil {
				ft.logf("error adding Mirage-Process rule: %v", err)
			} else {
				ft.mu.Lock()
				ft.didProcRule = true
				ft.mu.Unlock()
				ft.logf("added Mirage-Process rule in %v", d)
			}
		}
	}
	for _, cidr := range local {
		ft.logf("adding Mirage-In rule to allow %v ...", cidr)
		var d time.Duration
		d, err := ft.runFirewall("add", "rule", "name=Mirage-In", "dir=in", "action=allow", "localip="+cidr, "profile=private", "enable=yes")
		if err != nil {
			ft.logf("error adding Mirage-In rule to allow %v: %v", cidr, err)
			return err
		}
		ft.logf("added Mirage-In rule to allow %v in %v", cidr, d)
	}
