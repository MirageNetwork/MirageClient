...
func AddSeparator() *SeparatorItem {
	myid := addSeparator(atomic.AddUint32(&currentID, 1), 0)
...
func (item *MenuItem) AddSeparator() *SeparatorItem {
	myid := addSeparator(atomic.AddUint32(&currentID, 1), item.id)
	return &SeparatorItem{
		id:     myid,
		parent: item,
	}
...
// Hide hides a Separator
func (item *SeparatorItem) Hide() {
	hideSeparatorItem(item)
}

// Show shows a previously hidden menu item
func (item *SeparatorItem) Show() {
	showSeparatorItem(item)
}
